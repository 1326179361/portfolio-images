...
import { musicVideoVideos } from '../data/portfolio';

function MVCard({ item, index, isInView }: { 
  item: typeof musicVideoVideos[0]; 
  index: number; 
  isInView: boolean;
}) {
  return (
    ...
    >
      ...
      <div style={{
        ...
        >
          ...
          >
            <Play size={22} color="#7BB89A" fill="#7BB89A" strokeWidth={1.5} style={{ marginLeft: '3px' }} />
          </motion.div>
        </div>
      </div>
  ...
  return (
    <>
      <PageHero
        title="音画视频"
        subtitle="业余兴趣爱好"
        tags={["Cinematic", "Visual Storytelling", "Music Video", "Timelapse"]}
        bgImage={img("/images/heroes/hero-musicvideo.png")}
      />
      ...
      <section ref={ref} style={{
        ...
        <div style={{
          ...
          {musicVideoVideos.map((video, i) => (
            <MVCard key={video.id} item={video} index={i} isInView={isInView} />
          ))}
        </div>
